...
export default function PopupSelect({
  ...
  useEffect(() => {
    ...
    const updateMenuPosition = () => {
      ...
      const desiredWidth = Math.max(rect.width, portalMinWidth || 0);
      const nextWidth = Math.min(desiredWidth, portalMaxWidth, availableWidth);
  ...
  }, [isOpen, portal, portalMaxWidth, portalMinWidth]);
  ...
  const menu = isOpen ? (
    <div
      className={`${portal ? "fixed" : "absolute top-[calc(100%+8px)]"} z-80 rounded-2xl border border-novian-muted/35 bg-novian-surface/95 p-1.5 shadow-[0_18px_40px_rgba(0,0,0,0.28)] backdrop-blur ${portal ? "" : "w-full"} ${menuClassName}`}
      style={portal && menuStyle ? { top: menuStyle.top, left: menuStyle.left, width: menuStyle.width } : undefined}
      role="listbox"
    >
      {searchable ? (
  ...
  return (
    <div ref={rootRef} className="relative overflow-visible">
      ...
      {portal ? (menuStyle ? createPortal(menu, document.body) : null) : menu}
    </div>
  );
}
